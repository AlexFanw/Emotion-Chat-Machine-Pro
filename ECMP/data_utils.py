...
def prepare_data(data_dir, post_vocabulary_size, response_vocabulary_size, tokenizer=None):
    ...
    test_path = os.path.join(data_dir, config.get('data', 'raw_test_file'))
    ...
    if not gfile.Exists(tokenids_train_path) or not gfile.Exists(tokenids_dev_path):
        ...
        dev = json.load(open(dev_path, 'r', encoding='utf-8'))
        ...
        data_to_token_ids(test, post_vocab_path, response_vocab_path, tokenizer)
        ...
        with open(tokenids_test_path, 'w') as output:
            output.write(json.dumps(test, ensure_ascii=False))
